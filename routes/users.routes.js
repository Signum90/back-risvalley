...
router.post("/create/dashboard", Middlewares.validateAdminMiddleware, multerConfig.upload.single('logo'), [
  ...
  body('nombre').trim().notEmpty().withMessage(customMessages.required).isString().withMessage(customMessages.string).isLength({ max: 40 }),
  check('cargo').trim().notEmpty().isString().isLength({ max: 70 }),
  check('telefono').trim().optional({ nullable: true }).isInt().isLength({ max: 11 }),
  check('tipo').notEmpty().isInt({ min: 1, max: 4 }),
  ...
  check('telefonoEntidad').trim().isLength({ max: 11 }).custom(async (telefono, { req }) => {
    if (req.body.tipo != 1 && !telefono) return Promise.reject('El telefono de la entidad es obligatorio');
  }),
  check('emailEntidad').trim().isEmail().custom(async (email, { req }) => {
